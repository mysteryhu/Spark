...
object SparkStreaming06_Slide {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val SlideDStream: DStream[String] = lineStreams.window(Seconds(6),Seconds(3))
    //4. 将每一行数据切分,形成一个个单词
    val wordStreams: DStream[String] = SlideDStream.flatMap(_.split(" "))
    //5 类型转化(word,1L)
